...
<ul class="pagination" role="navigation">
    {{-- First Page Link --}}
    <!-- 最初のページへのリンク -->
    <li class="page-item {{ $paginator->onFirstPage() ? ' disabled' : '' }} d-none d-sm-block">
    ...
    @if ($paginator->currentPage() <= floor(config('const.PAGINATE.LINK_NUM') / 2)) 
        ...
        @endfor
            ...
            {{-- Last Page Link --}}
            <!-- 最後のページへのリンク -->
            <li class="page-item {{ $paginator->currentPage() == $paginator->lastPage() ? ' disabled' : '' }} d-none d-sm-block">
