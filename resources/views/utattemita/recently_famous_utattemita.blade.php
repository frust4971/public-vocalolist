    ...
    @for($i = 0;$i < count($utattemita);$i++)
        <div class="content px-2 py-lg-3 py-2">
            ...
            <div class="row">
                ...
                <h2 class="col-lg-6 bg-light py-2 border-left border-primary">{{htmlspecialchars_decode($utattemita[$i]->title,ENT_QUOTES)}}</h2>
            </div>
            <div class="row">

                <div class="col-lg-1 col-0"></div>
                <div class="col-lg-3 text-left">再生回数<div class="font-weight-bold">{{$utattemita[$i]->view_count}}</div></div>
                <div class="col-lg-3 text-right my-2">{{$utattemita[$i]->published_at}}</div>
            </div>
        </div>
    @endfor
    <?php $queries = array();
