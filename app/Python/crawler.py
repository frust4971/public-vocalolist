...
def is_vocalo_title(title):
    ...
    if re.match('.*オリジナル曲.*',title):
        return True
    not_vocalo_pattern = re.compile('.*(歌|cover|演奏|メドレー|ランキング|再生|合唱|太鼓|remix|曲|弾い|みた|バンド|クイズ|反応|検定|テスト|人力|diva|カバー|cover|mmd|mad|カラオケ|真似|プロセカ).*',re.IGNORECASE)
...
def is_utattemita_title(title):
    not_utattemita_pattern = re.compile('.*(ランキング|メドレー|替え歌|再生|feat|検定|クイズ|テスト|ボカロ曲).*',re.IGNORECASE)
