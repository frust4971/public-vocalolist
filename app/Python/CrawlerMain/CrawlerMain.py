...
if __name__ == "__main__":
    #最近話題のボカロを集める
    recently_vocalo_published_after = (datetime.datetime.today() + relativedelta(months=-24)).strftime('%Y-%m-%dT%H:%M:%S.%fZ')
    recently_vocalo_published_before = (datetime.datetime.today()).strftime('%Y-%m-%dT%H:%M:%S.%fZ')
    Crawler.crawlAndInsertToDB("recently_famous_vocalovideos", "ボカロ", True, "short", 100000, recently_vocalo_published_after, recently_vocalo_published_before)
    Crawler.crawlAndInsertToDB("recently_famous_vocalovideos","ボカロ",True,"medium",100000,recently_vocalo_published_after,recently_vocalo_published_before)
